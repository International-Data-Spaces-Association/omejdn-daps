...
class TokenGeneratorTest {
    ...
    fun configureSystemUnderTest() {
        ...
        now = Date().getTime() / 1000; // Divide by 1000 to get seconds
        iat = now;
        nbf = now;
        exp = now + 3600;
    }
    ...
    fun getAccessTokenAndVerifyES256() {
        ...
        if(iss.isEmpty()){
            println("Issuer3 argument is empty");
            System.exit(-1);
        }
        val sub : String =  iss;
    ...
    fun getAccessTokenAndVerifyES512() {
        ...
        if(iss.isEmpty()){
            println("Issuer4 argument is empty");
            System.exit(-1);
        }
        val sub : String =  iss;
    ...
    /* This test should fail because in the DAPS PS256 is not a supported algorithm */
    ...
    /* This test should fail because in the DAPS PS512 is not a supported algorithm */
    ...
    fun futureNBFToken() {
        ...
        // Variables to feed to the token
        val now : Long = Date().getTime() / 1000; // Divide by 1000 to get seconds
    ...
    /* The permission for an access_token does not depend on the context property, thus a
       DAT token is created with the default context if the authentication succeeds. */
    fun wrongContext() {
    ...
    /* The permission for an access_token does not depend on the type property, thus a
       DAT token is created with the default context if the authentication succeeds. */
    fun wrongType() {
    ...
    /* This test should fail. Would be a big vulnerability if a valid access_token
    ...
    /* https://www.chosenplaintext.ca/2015/03/31/jwt-algorithm-confusion.html This test should fail.
       The code in Client.extract_jwt_cid is responsible for this as it always checks
       ...
       would also result in the same behaviour as HMAC algorithms are not acccepted by the DAPS. */
    fun getAlgNone() {
    ...
    /* https://www.acunetix.com/blog/web-security-zone/what-are-json-injections/ Input is sanitized.
    ...
    /* This test should fail because the key used to sign the JWT does not match the signing key
